...
class NetworkSync {
    ...
    private final Pipeline.SyncType type;
            ...
            while (!terminate) {
                try {
                    ...
                    if (type == Pipeline.SyncType.CONTINUOUS && str.startsWith("SSI:SYNC:TIME")) //SSI format for compatibility
                    {
                        ...
                        System.arraycopy("SSI:SYNC:TIME".getBytes(StandardCharsets.US_ASCII), 0, data, 0, 13);
                ...
                if (frame.isRunning()) {
                    ...
                    for (int i = 0; i < NUM_REPETITIONS; i++) {
                        try {
                            ...
                            tmp[0] = frame.getTimeMs();
                            System.arraycopy("SSI:SYNC:RQST".getBytes(StandardCharsets.US_ASCII), 0, data, 0, 13);
